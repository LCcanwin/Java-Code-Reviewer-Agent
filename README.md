...
- **可选仓库索引上下文**：预留代码搜索/仓库索引 MCP Provider，用于补充定义、引用、测试和相关文件上下文
- **迭代反馈**：支持评审反馈校验和受限重试
...
| `context_retriever` | 从 Context Providers 收集并合并审查上下文，默认包含阿里巴巴规范，可选接入仓库索引 MCP |
...
`context_retriever` 使用可插拔 Provider 收集上下文，并通过 `ContextMerger` 合并到 `retrieved_context`：

| Provider | 默认 | 说明 |
|----------|------|------|
| `AlibabaRulesProvider` | 开启 | 基于现有 RAG 检索阿里巴巴 Java 规范 |
| `RepoIndexMCPProvider` | 关闭 | 预留代码搜索/仓库索引 MCP 适配器，接入后可提供定义、引用、相关测试、同包相似实现等上下文 |

MCP Provider 当前是轻量适配壳，不引入硬 MCP SDK 依赖。启用后需要注入满足以下方法的 client：
...
Provider 失败不会阻断主流程，错误会记录到 `context_errors`，审查会降级使用其他可用上下文。
...
│   ├── alibaba_rules.py     # 阿里规范 Provider
│   ├── repo_index_mcp.py    # 代码搜索/仓库索引 MCP Provider 壳
│   └── merger.py            # 多上下文合并与截断
├── templates/
│   └── index.html       # 前端页面
├── state/
...
context:
  ...
  providers:
    alibaba_rules:
      enabled: true
    repo_index_mcp:
      enabled: false            # 默认关闭，需要注入 MCP client 后开启
      server: code_search
      max_files: 20
...
| `REPO_INDEX_MCP_ENABLED` | 否 | 是否启用仓库索引 MCP Provider |
| `REPO_INDEX_MCP_SERVER` | 否 | 仓库索引 MCP server 名称 |
